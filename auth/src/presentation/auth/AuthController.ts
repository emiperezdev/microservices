...
export class AuthController {
  ...
  login = async (req: Request, res: Response) => {
    ...
    try {
      ...
      return res
        .status(500)
        .json({ message: ">>> [AUTH SERVICE] Server error" });
    }
  };
  ...
  me = async (req: CustomRequest, res: Response) => {
    try {
      ...
      if (!userEmail)
        return res
          .status(400)
          .json({ message: "[AUTH SERVICE] User email not found in token" });
      ...
      if (!user)
        return res
          .status(404)
          .json({ message: "[AUTH SERVICE] User not found" });
    ...
    try {
      ...
      if (!isMatch)
        return res
          .status(400)
          .json({
            message: ">>> [AUTH SERVICE] Current password is incorrect",
          });
      ...
      return res
        .status(200)
        .json({ message: ">>> [AUTH SERVICE] Password updated successfully" });
    } catch (error) {
